...
func uptade_text():
	$RichTextLabel.text = '[center][b]Mirror 5[/b]
	Adds your score read backwards. ' + str(global.mirror_chance) + ' % chance of not breaking[/center]'

	$".".tooltip_text = 'Time for some self reflection'

func _on_popcorn_button_pressed():
	if global.money >= 5:
		global.money -= 5
		$"../../../inventory".inventory.append ("res://mirror.tscn")
		$".".visible = false
	else:
